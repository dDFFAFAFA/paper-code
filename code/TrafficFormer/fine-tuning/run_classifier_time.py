...
def evaluate(args, dataset, print_confusion_matrix=False):
    ...
    return correct / len(dataset), f1_score(y_true,y_pred,average='macro'), confusion, inference_time
...
def main():
    ...
    for i in range(5):
        ...
        if args.test_path is not None:
            ...
            train_val_time_list.append(train_end_time - start_training_time)
            test_time_list.append(result[3])
            with open(results_file_path, "a") as results_file:
                json.dump(results, results_file)
                results_file.write("\n")

    print("Training + Valication time: ", train_val_time_list)
    print("Training + Valication time: ", np.mean(train_val_time_list))
    print("Test time: ", test_time_list)
    print("Test time: ", np.mean(test_time_list))
